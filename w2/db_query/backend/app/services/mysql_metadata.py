...
def _format_mysql_metadata_error(error: Exception) -> str:
    """
    Format MySQL metadata extraction error into user-friendly message.

    Args:
        error: Exception raised during metadata extraction
    ...
    # Permission errors
    if "access denied" in error_lower or "1142" in error_str:
        return (
            "Access denied: Insufficient permissions to access INFORMATION_SCHEMA. "
            "Please ensure your MySQL user has SELECT permissions on INFORMATION_SCHEMA."
        )

    # Connection errors
    if "can't connect" in error_lower or "lost connection" in error_lower:
        return (
            "Connection lost during metadata extraction. "
            "Please try again or check your MySQL server connection."
        )

    # Generic error
    return f"Failed to extract MySQL metadata: {error_str}"
...
async def extract_metadata(database_name: str, pool: aiomysql.Pool) -> Dict[str, Any]:
    ...
    try:
        ...
        error_msg = _format_mysql_metadata_error(e)
        logger.error(f"MySQL metadata extraction failed for {database_name}: {error_msg}")
